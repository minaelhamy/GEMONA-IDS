...
class ProfileController extends GetxController {
  ...
  getProfile() async {
    ...
    accountBalance.value = double.parse(
      profileModel?.data?.balance.toString() ?? "0",
    );
  ...
  Future getImageFromGallary() async {
    ...
    if (imageSize > 2) {
      customSnackbar(
        "ERROR".tr,
        "IMAGE_SHOULD_BE_LESS_THAN_2MB".tr,
        AppColor.error,
      );
    } else {
  ...
  updateProfileImage(file) async {
    isLoading(true);
    update();
    await server.multipartRequest(ApiList.changeImage, file).then((
      response,
    ) async {
      if (response != null) {
        isLoading(false);
  ...
  updateUserProfile(context) async {
    ...
    await server
        .putRequest(endPoint: ApiList.profileUpdate, body: jsonBody)
        .then((response) async {
          ...
          if (response != null && response.statusCode == 200) {
            ...
            customSnackbar(
              "ERROR".tr,
              jsonResponse['message'].toString().tr,
              AppColor.error,
            );
          } else {
            isLoading(false);
