...
class _ProfileScreenState extends State<ProfileScreen> {
                    ...
                    child: Column(
                      children: [
                        ...
                        Obx(
                          () => SizedBox(
                            child:
                                profile.pagesMap.isNotEmpty &&
                                    profile.pagesModel?.data != null
                                ? ListView.builder(
                                    shrinkWrap: true,
                                    physics:
                                        const NeverScrollableScrollPhysics(),
                                    itemCount: profile.pagesModel!.data!
                                        .where((page) => page.status == 5)
                                        .toList()
                                        .length,
                                    itemBuilder: (BuildContext context, index) {
                                      final activePages = profile
                                          .pagesModel!
                                          .data!
                                          .where((page) => page.status == 5)
                                          .toList();
                                      return Column(
                                        children: [
                                          profileItem(
                                            activePages[index].description,
                                            SvgIcon.tc,
                                            activePages[index].title,
                                            activePages[index].image ?? "",
                                          ),
                                          DeviderWidget(),
                                        ],
                                      );
  ...
  InkWell profileItem(des, icon, textValue, image) {
    return InkWell(
      onTap: () => Get.to(
        () => PagesScreen(tittle: textValue, description: des, image: image),
      ),
      child: SizedBox(
        height: 52.h,
