...
class ProductDetailsController extends GetxController {
  ...
  final relatedProductList = <RelatedProduct>[].obs;
  ...
  Future<void> fetchProductDetails({required String slug}) async {
    isLaoding.value = 1;
    final data = await RemoteServices().fetchProductDetails(
      slug: slug,
      reviewLimit: reviewLimit.value,
    );
  ...
  Future<void> fetchRelatedProduct({required String slug}) async {
    ...
    data.fold((error) => error, (relatedProduct) {
      ...
      relatedProductList.value = relatedProductModel.value.data ?? [];
      refresh();
    });
  }

  Future<void> fetchInitialVariation({required String productId}) async {
    final data = await RemoteServices().fetchInitialVariation(
      productId: productId,
    );
  ...
  Future<void> fetchChildrenVariation1({
    ...
    final data = await RemoteServices().fetchChildrenVariation(
