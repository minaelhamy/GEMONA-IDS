...
class ReviewController extends GetxController {
  ...
  getReviewDetails({required String review_id}) async {
    ...
    if (reviewDetailsMap.isNotEmpty) {
      ...
      List<String> images = reviewDetailsModel!.data!.images ?? [];
        ...
        .then((response) async {
          response.stream.transform(utf8.decoder).listen((value) {
            if (response.statusCode == 201) {
              ...
              customSnackbar(
                "SUCCESS".tr,
                'Review submitted successfully!'.toString().tr,
                AppColor.success,
              );
              ...
              customSnackbar(
                "ERROR".tr,
                'Review not submitted'.toString().tr,
                AppColor.error,
              );
            }
          });
  ...
  reviewUpdate({
    ...
    await server
        .putRequest(endPoint: ApiList.updateReview + review_id, body: jsonBody)
        .then((response) async {
  ...
  updateImage(file, reviewId) async {
    update();
    await server
        .multipartRequestReviewUpdate(ApiList.updateImage + reviewId, file)
        .then((response) async {
          response.stream.transform(utf8.decoder).listen((value) {
            if (response == 200) {
              update();
              getReviewDetails(review_id: reviewId);
              update();
            } else {
              update();
            }
  ...
  deleteImage({required String review_id, required String index}) async {
    var response;
    response = await ReviewRepo().deleteImage(
      review_id: review_id,
      index: index,
    );
